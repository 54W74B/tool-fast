...
import com.intters.util.date.DateException;
import com.intters.util.date.DatePattern;
import com.intters.util.date.DateTime;
import com.intters.util.date.DateUtil;
...
public class IdcardUtil {
     ...
     * @return 日期
     */
    public static DateTime getBirthDate(String idCard) throws ParseException {
        final String birthByIdCard = getBirthByIdCard(idCard);
        return null == birthByIdCard ? null : DateUtil.parse(birthByIdCard);
    }
    ...
    public static int getAgeByIdCard(String idCard, Date dateToCompare) {
        ...
        try {
            return DateUtil.age(DateUtil.parse(birth), dateToCompare);
        } catch (ParseException e) {
            throw new DateException(String.format("Parse [%s] error!", idCard), e);
        }
    }
    ...
    public static String convert15To18(String idCard) {
        ...
        if (NumberUtil.isNumber(idCard)) {
            ...
            Date birthDate = null;
            try {
                birthDate = DateUtil.parse(birthday);
            } catch (ParseException e) {
                throw new DateException(String.format("Parse [%s] error!", birthDate), e);
            }
            // 获取出生年(完全表现形式,如：2010)
            int sYear = DateUtil.set(birthDate).year();
            if(sYear > 2000) {
                //2000年之后不存在15位身份证号，此处用于修复此问题的判断
                sYear -= 100;
            }

            idCard18 = StrUtil.splice(idCard.substring(0, 6), String.valueOf(sYear) ,idCard.substring(8));
            // 获取校验位
            char sVal = getCheckCode18(idCard18.toString());
            idCard18 = StrUtil.splice(idCard18, String.valueOf(sVal));
        } else {
            return null;
        }
